...
from src.db_utils.bulk import BulkInserter, bulk_insert_csv
...
def process_data_file(
    file_path: str, table_name: str, max_rows: Optional[int] = None
) -> bool:
    """
    ...
    try:
        ...
        logger.info(
            f"Cleaned data: {len(cleaned_df)} rows, {len(cleaned_df.columns)} columns"
        )
        ...
        success = inserter.insert_dataframe(
            df=cleaned_df, table_name=table_name, if_exists="append"
        )

        if success:
            logger.info(
                f"✅ Successfully processed {len(cleaned_df)} records into {table_name}"
            )
            return True
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Migrate data with cleaning and safe insertion"
    )
    parser.add_argument("--file", type=str, help="Specific CSV file to process")
    parser.add_argument(
        "--table", type=str, help="Target table name (required if --file is used)"
    )
    parser.add_argument(
        "--all", action="store_true", help="Process all configured data files"
    )
    parser.add_argument(
        "--max-rows", type=int, help="Maximum rows to process per file (for testing)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
