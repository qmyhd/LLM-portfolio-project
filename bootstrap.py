...
IMPORTANT: This is NOT a traditional setup.py for package building!
This script is an environment bootstrap tool for project setup.

Package building is handled by pyproject.toml using setuptools build backend.
This script handles: virtual environment, dependencies, directories, and validation.
...
class EnvironmentSetup:
    ...
    def install_dependencies(self, dev_mode: bool = False):
        ...
        if requirements_file.exists():
            ...
            print_status(
                "requirements.txt not found, installing minimal dependencies", "WARNING"
            )
...
"""
        ...
        print_status(
            "Please edit .env with your actual API keys and configuration", "WARNING"
        )
    ...
    def run_setup(
        self, quick: bool = False, dev_mode: bool = False, check_only: bool = False
    ):
...
def main():
    ...
    parser.add_argument(
        "--check-only", action="store_true", help="Only run health checks"
    )
    ...
    setup.run_setup(quick=args.quick, dev_mode=args.dev, check_only=args.check_only)
