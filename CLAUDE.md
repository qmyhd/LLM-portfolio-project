...
Consolidated schema in `schema/` (060_baseline_current.sql + 061-071 incremental, older files archived). 23 active tables. Recent migrations: 062 (stock_notes), 063 (discord_ingest_cursors), 064 (user_ideas), 065 (account_balances PK), 066 (accounts connection_status), 067 (stock_analysis_cache + portfolio_risk_cache), 068 (position_snapshots), **069 (accounts.bucket strategy classification — long_term/swing/day/retirement/other), 070 (portfolio_risk_cache bucket in PK), 071 (stock_analysis_cache bucket in PK so multi-agent analysis caches per-bucket)**. Core tables: `discord_messages`, `discord_parsed_ideas`, `ohlcv_daily`, `positions`, `orders`, `accounts`, `account_balances`, `activities`, `user_ideas`, `twitter_data`, `stock_profile_current`, `stock_notes`, `discord_ingest_cursors`, `stock_analysis_cache`, `portfolio_risk_cache`, `position_snapshots`. All tables have RLS enabled — service role key required in DATABASE_URL.
