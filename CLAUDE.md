...
- **`src/logging_utils.py`** — `log_message_to_database()`, the live `on_message` writer. Extracts `tweet_urls`, captures Discord `embeds` (unfurled tweet text — the supported path since the Twitter API tier no longer allows tweet reads), and pre-classifies `parse_status` at ingest (bot/command/empty/≤10-char → `skipped`; tweet/embed messages stay `pending`) so non-content never sits in the parse queue. Never overwrites an existing `parse_status` on re-ingest. The NLP parser (`scripts/nlp/parse_messages.py`) enriches parseable text with embed tweet content.
...
Consolidated schema in `schema/` (060_baseline_current.sql + 061-077 incremental, older files archived). ~33 active tables. Recent migrations: 069 (accounts.bucket strategy classification), 070-071 (bucket in risk/analysis cache PKs), **072 (stock_thesis_profiles + revisions), 073 (source credibility: people, source_identities, tiers, stock_topic_tags), 074 (video_quotes), 075 (app_users + Google auth, richer user_ideas provenance/curation, discord_parsed_ideas curation columns, philosophy/religion/arts tiers), 076 (security hardening: dropped the SECURITY DEFINER helper, revoked anon/authenticated grants, dropped 4 dead tables — trade_history/discord_trading_clean/discord_market_clean/discord_idea_units), 077 (discord_messages.embeds column, backfilled tweet_urls, reconciled non-parseable pending → skipped)**. Core tables: `discord_messages`, `discord_parsed_ideas`, `ohlcv_daily`, `positions`, `orders`, `accounts`, `account_balances`, `activities`, `user_ideas`, `stock_profile_current`, `stock_notes`, `discord_ingest_cursors`, `stock_analysis_cache`, `portfolio_risk_cache`, `position_snapshots`, `stock_thesis_profiles`, `people`, `source_identities`, `video_quotes`, `app_users`. All tables have RLS enabled (deny-by-default; anon/authenticated grants revoked in 076) — service role key required in DATABASE_URL; all app data flows through FastAPI, never the anon key.

### FastAPI Routes (`app/routes/`)

19 route files: `portfolio.py` (positions, sync, movers, sparklines, return-series, equity-curve), `orders.py`, `stocks.py` (profile incl. sector/industry, ideas, OHLCV), `openbb.py` (transcripts, fundamentals, filings, news, notes), `analysis.py` (multi-agent stock analysis, portfolio risk incl. sector exposure), `trades.py` (unified trade feed with P/L enrichment), `chat.py`, `search.py`, `watchlist.py`, `ideas.py` (CRUD, refine, context, `/timeline`, `/import/messages`, `/discord-parsed` review-queue list + `/discord-parsed/{id}/curation`), `activities.py`, `connections.py` (`PATCH /connections/{id}/bucket`), `sentiment.py` (feed, status, `/reparse` — skips human-reviewed rows), `people.py` + `profiles.py` + `credibility.py` (source-credibility / thesis-profile system), `videos.py` (YouTube transcript + quotes), `webhook.py`, `debug.py` (opt-in via `DEBUG_ENDPOINTS=1`). Plus `app/auth.py` router: `POST /auth/google` (verifies Google ID token, upserts `app_users`, OWNER_EMAILS→owner else viewer), `GET /auth/config` (secrets-free config health check). All routers carry `Depends(require_api_key)`.

**Bucket filtering (`?bucket=<long_term|swing|day|retirement|other|all>`)**: every endpoint that returns positions, trades, activities, orders, risk, or analysis accepts an optional bucket query param. Endpoints with bucket support: `/portfolio`, `/portfolio/movers`, `/portfolio/sparklines`, `/portfolio/equity-curve`, `/portfolio/risk` (cached per bucket via `portfolio_risk_cache` composite PK), `/orders`, `/activities`, `/trades/recent`, `/stocks/{ticker}/trades`, `/stocks/{ticker}` (position metrics + order counts), `/stocks/{ticker}/ohlcv` (trade-marker overlay), `/stocks/{ticker}/activities`, `/stocks/{ticker}/chat` (position context), `/stocks/{ticker}/analysis*` (multi-agent analysis with bucket-scoped position context, cached per-bucket via `stock_analysis_cache` composite PK). Implementation pattern: `bucket = validate_bucket(bucket); clause, bp = bucket_filter_sql(bucket, alias="acc")` then interpolate `{clause}` into the WHERE and merge `bp` into params. New JOINs use `LEFT JOIN accounts acc ON ...` so legacy rows with orphan `account_id` keep surfacing when no bucket filter is set.
