...
**Purpose:** Load secrets from AWS Secrets Manager for EC2/Lambda deployment
...
**Purpose:** Serverless functions for scheduled data processing

#### Directory Structure
```
lambdas/
├── shared/                    # Common utilities
│   └── __init__.py           # load_secrets, validate_db_connection
├── snaptrade_sync/
│   └── handler.py            # SnapTrade data sync (every 30 min)
├── discord_nlp/
│   └── handler.py            # NLP parsing (every 15 min)
└── ohlcv_backfill/
    └── handler.py            # OHLCV daily bars (daily at 5:30 PM ET)
```

#### Lambda: SnapTrade Sync

```python
def handler(event: Dict, context: Any) -> Dict:
    """
    Triggered by EventBridge every 30 min during market hours.
    
    Flow:
        1. Load secrets from AWS Secrets Manager
        2. Validate database connection
        3. Call SnapTradeCollector.collect_all_data()
        4. Return sync results
    
    Schedule: cron(0/30 13-21 ? * MON-FRI *)  # 9-5 ET
    """
```
...
def handler(event: Dict, context: Any) -> Dict:
    ...
    Flow:
        1. Load secrets (main + RDS)
        2. Fetch pending messages (parse_status IS NULL)
        3. For each message:
           - Call process_message() → returns Dict
           - Save via save_parsed_ideas_atomic()
        4. Return processing stats
    
    Key Detail:
        process_message() returns Dict with keys:
        - status: "ok" | "noise" | "error" | "skipped"
        - ideas: List[Dict]
        - model: str
...
#### Lambda: OHLCV Backfill

```python
def handler(event: Dict, context: Any) -> Dict:
    """
    Triggered by EventBridge daily at 5:30 PM ET.
    
    Flow:
        1. Load secrets
        2. Validate RDS connection (fallback to Supabase)
        3. Fetch daily bars via DatabentoCollector.fetch_daily_bars()
        4. Store via store_ohlcv_rds_dataframe() or store_ohlcv_supabase_dataframe()
    
    Key Detail:
        fetch_daily_bars() returns pandas DataFrame with columns:
...
**Purpose:** Unified orchestrator for EC2 cron jobs

```python
def main():
    """
    Run daily data pipeline tasks.
    
    Tasks:
        --snaptrade: Sync brokerage data
        --discord: Process NLP on pending messages  
        --ohlcv: Backfill OHLCV bars
        (default: run all)
    
    Features:
        - File-based locking (prevents concurrent runs)
        - AWS Secrets Manager integration
        - Status tracking in JSON file
        - Graceful error handling (continues on failure)
    """

def run_discord_processing(dry_run: bool = False) -> dict:
    """
    Process unprocessed Discord messages through NLP.
    
    Uses:
        - get_pending_messages() to fetch messages
        - parse_single_message() to parse each one
    
    Returns:
        Dict with messages_processed, ideas_extracted, errors
    """
...
def run_ohlcv_backfill(dry_run: bool = False, days: int = 5) -> dict:
    ...
    Uses:
        - DatabentoCollector.get_all_tracked_symbols()
        - DatabentoCollector.run_backfill()
    
    Returns:
        Dict with rows_fetched, rows_saved, errors
    """
```

---

### 📄 Backfill Script (`scripts/backfill_ohlcv.py`)

**Purpose:** CLI for historical OHLCV backfill

```bash
# Usage examples
python scripts/backfill_ohlcv.py --daily           # Last 5 days
python scripts/backfill_ohlcv.py --full            # Full history (2023-03-28+)
...
def main():
    """
    CLI entry point for OHLCV backfill.
    
    Features:
        - Automatic symbol detection from portfolio
        - Date range splitting for dataset changes
        - Multi-storage: RDS, S3, Supabase
