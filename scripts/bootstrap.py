...
class BootstrapManager:
    ...
    def install_dependencies(self, force=False):
        ...
        try:
            ...
            if not venv_active:
                logger.warning("⚠️ No virtual environment detected. Consider using a virtual environment.")
    ...
    def load_environment(self):
        ...
        try:
            ...
            logger.info(f"Database URL configured: {'Yes' if config.DATABASE_URL or database_url else 'No'}")
            logger.info(f"Supabase configured: {'Yes' if config.SUPABASE_URL else 'No'}")
    ...
    def test_database_connection(self):
        ...
        try:
            ...
            if connection_info and connection_info.get("status") == "connected":
                ...
                error = connection_info.get("error", "Unknown error") if connection_info else "No response"
    ...
    def check_migration_needed(self):
        ...
        try:
            ...
            if not database_url.startswith("sqlite"):
                ...
                sqlite_path = self.project_root / "data" / "database" / "price_history.db"
                if sqlite_path.exists():
                    sqlite_size = sqlite_path.stat().st_size
                    if sqlite_size > 1024:  # More than 1KB suggests real data
                        logger.info(f"📊 SQLite database found with {sqlite_size} bytes - pgloader migration needed")
                        return True
    ...
    def run_migration(self):
        ...
        try:
            ...
            try:
                result = subprocess.run(["pgloader", "--version"], capture_output=True, text=True)
                if result.returncode != 0:
                    logger.error("❌ pgloader not found. Please install pgloader first.")
                    logger.info("Installation: apt-get install pgloader (Ubuntu) or brew install pgloader (macOS)")
                    return False
                logger.info(f"✅ pgloader found: {result.stdout.strip()}")
            except FileNotFoundError:
                logger.error("❌ pgloader not found in PATH. Please install pgloader first.")
                return False
            ...
            if result.returncode == 0:
                ...
                self.migration_flag.write_text(f"Migration completed on {datetime.now()}")
    ...
    def schedule_data_updates(self):
        ...
        try:
            ...
            update_interval = getattr(config, 'DATA_UPDATE_INTERVAL', 7200)  # 2 hours in seconds
            
            logger.info(f"📊 Data updates scheduled every {update_interval//3600} hours")
    ...
    def run_health_checks(self):
        ...
        try:
            ...
            from src.config import settings
            config = settings()
            config_healthy = bool(config.DATABASE_URL and getattr(config, 'SUPABASE_URL', None))
            logger.info(f"Configuration: {'✅ Complete' if config_healthy else '⚠️ Missing keys'}")
    ...
    def bootstrap(self):
        ...
        try:
            ...
            logger.info("="*60)
            logger.info(f"🎉 Bootstrap completed successfully in {duration.total_seconds():.1f}s")
