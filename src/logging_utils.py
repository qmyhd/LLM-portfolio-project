    ...
    try:
        ...
        try:
            ...
            message_data = {
                ...
                'author_id': str(message.author.id) if hasattr(message.author, 'id') else None,
                ...
                'mentions': ', '.join([str(user.id) for user in message.mentions]) if message.mentions else ''
            }
            ...
            if success:
                ...
                logger.warning("⚠️ Failed to log to Supabase, falling back to unified database layer")
