...
def analyze_position_history(
            ...
            if running_shares > 0:
                ...
                if running_shares > 0:
                    ...
                    total_cost = total_cost * (
                        running_shares / (running_shares + shares)
                    )
                else:
                    total_cost = 0
        ...
        cost_basis_timeline.append(
            {
                "date": trade["execution_date"],
                "avg_cost_basis": (
                    total_cost / running_shares if running_shares > 0 else 0
                ),
                "total_cost": total_cost,
            }
        )
    ...
    unrealized_pnl = (
        (current_price - current_cost_basis) * running_shares
        if running_shares > 0
        else 0
    )
    ...
    analysis = {
        ...
        "position_summary": {
            ...
            "total_invested": sum(
                buy_trades["execution_price"] * buy_trades["total_quantity"]
            ),
            "total_divested": sum(
                sell_trades["execution_price"] * sell_trades["total_quantity"]
            ),
            "realized_pnl": total_realized_pnl,
        ...
        "trading_activity": {
            ...
            "avg_buy_price": (
                buy_trades["execution_price"].mean() if not buy_trades.empty else 0
            ),
            "avg_sell_price": (
                sell_trades["execution_price"].mean() if not sell_trades.empty else 0
            ),
            "largest_buy": (
                buy_trades["total_quantity"].max() if not buy_trades.empty else 0
            ),
            "largest_sell": (
                sell_trades["total_quantity"].max() if not sell_trades.empty else 0
            ),
        },
        "timeline_data": {
...
def get_stored_positions():
    """Get all stored positions from the positions table."""
    try:
        from src.db import execute_sql
...
def create_enhanced_chart_annotations(position_analysis: Dict) -> List[Dict]:
    ...
    for i, point in enumerate(timeline):
        ...
        if (
            i == 0
            or abs(point["position_size"] - timeline[i - 1]["position_size"]) > 50
    ...
    cost_timeline = position_analysis.get("timeline_data", {}).get(
