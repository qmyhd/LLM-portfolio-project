    ...
    if database_url and database_url.startswith("postgresql"):
        try:
            ...
            if not config.ALLOW_SQLITE_FALLBACK:
                raise RuntimeError(
                    "DATABASE_URL is configured but connection failed, and SQLite fallback is not allowed"
                )
...
def initialize_database():
    ...
    if use_postgres():
        ...
        try:
            ...
            -- SnapTrade processed data tables
            ...
            try:
                ...
                logging.info(
                    "Database initialized successfully with all tables (social and financial data)"
                )
                return True
            except Exception:
                conn.rollback()
