...
class _Settings(BaseSettings):
    ...
    SQLITE_PATH: str = (
        "data/database/price_history.db"  # SQLite database path for migration and fallback
    )
...
def settings() -> _Settings:
    ...
    # Apply field mapping from environment variables if needed
    import os
...
def get_database_url() -> str:
    ...
    if config.SUPABASE_URL and config.SUPABASE_SERVICE_ROLE_KEY:
        ...
        if ":6543" in config.SUPABASE_URL:
            ...
            pooler_url = config.SUPABASE_URL.replace("https://", "").replace(
                "http://", ""
            )
            ...
            direct_url = config.SUPABASE_URL.replace("https://", "").replace(
                "http://", ""
            )
    ...
    raise ValueError(
        "No database configuration found. Set DATABASE_URL or configure Supabase settings."
    )
