...
- PostgreSQL backend with connection pooling optimization
...
class BulkInserter:
        ...
        try:
            ...
            for batch_num, batch_df in enumerate(self._batch_dataframe(df), 1):
                ...
                if success:
                    ...
                    logger.debug(
                        f"Batch {batch_num}/{total_batches} completed ({len(batch_df)} rows)"
                    )
            ...
            logger.info(
                f"Successfully inserted {inserted_rows} rows to PostgreSQL table {table_name}"
            )
            return True
    ...
    def _insert_batch_individually(
        self, batch_df: pd.DataFrame, table_name: str
    ) -> bool:
        """Insert batch rows individually, skipping failures."""
        success_count = 0

        for idx, row in batch_df.iterrows():
            single_row_df = batch_df.iloc[
                [idx - batch_df.index[0]]
        ...
        logger.info(
            f"Individual insert: {success_count}/{len(batch_df)} rows succeeded"
        )
...
def bulk_insert_csv(
    csv_path: Path, table_name: str, batch_size: int = 1000, clean_first: bool = True
) -> bool:
    """
    ...
    try:
        ...
        return inserter.insert_dataframe(
            df, table_name, progress_callback=progress_callback
        )
