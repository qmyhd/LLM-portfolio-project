...
def register(bot: commands.Bot):
    ...
    async def process_channel_data(ctx, channel_type: str = "general"):
        ...
        try:
            await ctx.send(
                f"🔄 Processing Discord data for #{ctx.channel.name} as {channel_type} channel..."
            )
            ...
            await ctx.send(
                f"❌ **Error processing channel data**\n"
                f"• Error: {str(e)}\n"
    ...
    async def channel_stats_cmd(ctx):
        ...
        try:
            ...
            if not stats or all(v == 0 for v in stats.values()):
                await ctx.send(
                    "ℹ️ No processed channel data found. Use `!process` to process current channel first."
                )
                return

            await ctx.send(
                f"📊 **Channel Statistics: #{ctx.channel.name}**\n"
                f"• Raw Messages: {stats['raw_messages']}\n"
                f"• Market Processed: {stats['market_processed']}\n"
    ...
    async def global_stats_cmd(ctx):
        ...
        try:
            ...
            if not stats or all(v == 0 for v in stats.values()):
                await ctx.send(
                    "ℹ️ No processed data found. Use `!process` to process channels first."
                )
                return
            ...
            await ctx.send(
                ...
                f"• Total Market Processed: {stats['market_processed']}\n"
            ...
            await ctx.send(
                f"❌ **Error fetching global statistics**\n"
                f"• Error: {str(e)}\n"
                f"• Please check logs and try again"
            )
