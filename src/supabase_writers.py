            ...
            with self.engine.begin() as conn:
                for order in orders_data:
                    ...
                    order_data = {
                        ...
                        "execution_price": self._safe_decimal(
                        ...
                        "time_executed": self._safe_timestamp(
                            order.get("time_executed")
                        ),
                        "universal_symbol": (
                            json.dumps(order.get("universal_symbol"))
                            if order.get("universal_symbol")
                            else None
                        ),
                        "option_symbol": (
                            json.dumps(order.get("option_symbol"))
            ...
            with self.engine.begin() as conn:
                ...
                """
                    ),
                    {
                        ...
                        "tickers_detected": str(
                            message_data.get("tickers_detected", "")
                        ),
                ...
                logger.debug(
                    f"✅ Wrote Discord message {message_data.get('message_id')} to Supabase"
                )
                return True
                ...
                for price in price_data:
                    ...
                    """
                        ),
                        {
                            ...
                            "previous_close": self._safe_decimal(
                                price.get("previous_close")
                            ),
                            "abs_change": self._safe_decimal(price.get("abs_change")),
                            "percent_change": self._safe_decimal(
                                price.get("percent_change")
                            ),
                            "timestamp": self._safe_timestamp(price.get("timestamp"))
                            or datetime.now(),
                            "user_id": "default_user",
                            "created_at": datetime.now(),
                        },
