...
class TestTickerExtraction(unittest.TestCase):
    ...
    def test_ticker_length_limits(self):
        ...
        self.assertEqual(extract_ticker_symbols(text), ["$A", "$AB", "$ABC", "$ABCD", "$ABCDE", "$ABCDEF"])
    
    def test_non_word_boundary(self):
        """Test tickers followed by non-word boundaries"""
        text = "$AAPL. $MSFT, $TSLA: $NVDA;"
        self.assertEqual(extract_ticker_symbols(text), ["$AAPL", "$MSFT", "$TSLA", "$NVDA"])
...
class TestPromptBuilder(unittest.TestCase):
    ...
    def test_format_prices_as_json(self):
        ...
        self.assertAlmostEqual(aapl['percent_change'], 3.4482758620689653)  # (150/145 - 1) * 100
        
        msft = prices['prices'][1]
        self.assertEqual(msft['net_change'], -5)  # 200 - 205
        self.assertAlmostEqual(msft['percent_change'], -2.4390243902439024)  # (200/205 - 1) * 100
    ...
    def test_create_journal_prompt(self):
        ...
        empty_prompt = create_journal_prompt(pd.DataFrame(), pd.DataFrame(), pd.DataFrame())
